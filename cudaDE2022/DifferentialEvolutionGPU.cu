...
//
// Once you have a cost function find the costFunc function, and add into preprocessor directives switch statement
//
...
void printCudaVector(float* d_vec, int size){
	...
	std::cout << "{";
	for(int i = 0; i < size; i++)  std::cout << h_vec[i] << ", ";
	std::cout << "}" << std::endl;
	delete[] h_vec;
}
...
// ----------------------------------------------------------------------------------------------------------------------------# @blk1
// IMPORTANT
// costFunc - this function must implement whatever cost function is being minimized. Feel free to delete all code in here.
// This is a bit of a hack and not elegant at all. The issue is that CUDA doesn't support function passing device code between host software.
// There is a possibilty of using virtual functions, but was concerned that the polymorphic function have a lot of overhead So instead use this super ugly method for changing the cost function.
// @param vec - the vector to be evaulated
// @param args - a set of user arguments

__device__ __host__ float quadraticFunc(const float* vec, const void* args){
	struct data* a = (struct data*)args;
	float res = 0;
...
__device__ __host__ float costWithArgs(const float* vec, const void* args){
	struct data* a = (struct data*)args;
	float        x = vec[0];
	float        y = vec[1];
	return x*x + y*y + 9 - (6*x) + a->arr[1] + a->v;
}
__device__ __host__ float costFunctionWithManyLocalMinima(const float* vec, const void* args){
	float x = vec[0];
	float y = vec[1];
	return -(cos(x) + cos(y)) + 0.2*(x*x) + 0.2*(y*y);
}
__device__ __host__ float cost3D(const float* vec, const void* args){
	float x = vec[0]-3;
	float y = vec[1]-1;
	float z = vec[2]+3;
	return (x*x*x*x)- (2*x*x*x) + (z*z*z*z) + (y*y*y);
}
__device__ __host__ float costRastrigin(const float* vec, const void* args){  // http://sfu.ca/~ssurjano/rastr.html
	struct data* a = (struct data*)args;
	float res = 10*a->dim;
	for(int i=0; i<a->dim; ++i){
		float xi = vec[i];
		res     += xi*xi - 10*cos(2*M_PI*xi);
	}
	return res;
}
...
// ----------------------------------------------------------------------------------------------------------------------------# @blk1
__global__ void generateRandomVectorAndInit(float* d_x, float* d_min, float* d_max, float* d_cost, void* costArgs, curandState_t* randStates, int popSize, int dim, unsigned long seed){
	int idx = (blockIdx.x * blockDim.x) + threadIdx.x;
	if(idx >= popSize) return;

	curandState_t* state = &randStates[idx];
	curand_init(seed, idx,0,state);
	for(int i = 0; i < dim; i++)  d_x[(idx*dim) + i] = (curand_uniform(state) * (d_max[i] - d_min[i])) + d_min[i];
...
__global__ void evolutionKernel(float* d_target, float* d_trial, float* d_cost, float* d_target2, float* d_min, float* d_max, curandState_t* randStates, int dim, int popSize, int CR, float F, void* costArgs){  // @arg CR  // Must be given as value between [0,999]
	int idx = (blockIdx.x * blockDim.x) + threadIdx.x;
	if(idx >= popSize) return; // stop executing this block if all populations have been used
	curandState_t* state = &randStates[idx];
	...
	///////////////////// MUTATION ////////////////
	for(int k = 1; k <= dim; k++){
		if((curand(state)%1000)<CR || k==dim)  d_trial[(idx*dim)+j] = d_target[(a  *dim)+j] + F * (d_target[(b*dim)+j] - d_target[(c*dim)+j]);  // trial vector param comes from vector plus weighted differential
		else                                   d_trial[(idx*dim)+j] = d_target[(idx*dim)+j];                                                    // end if else for creating trial vector
		j = (j+1) % dim;
	} // end for loop through parameters

	float score = costFunc(&d_trial[idx*dim], costArgs);
	if(score < d_cost[idx]){  // copy trial into new vector
		for(j = 0; j < dim; j++)
			d_target2[(idx*dim) + j] = d_trial[(idx*dim) + j];  // printf("idx = %d, d_target2[%d] = %f, score = %f\n", idx, (idx*dim)+j, d_trial[(idx*dim) + j], score);
		d_cost[idx] = score;
	}else  // copy target to the second vector
		for(j = 0; j < dim; j++)
			d_target2[(idx*dim) + j] = d_target[(idx*dim) + j];  // printf("idx = %d, d_target2[%d] = %f, score = %f\n", idx, (idx*dim)+j, d_trial[(idx*dim) + j], score);
}
...
void differentialEvolution(float* d_target, float* d_trial, float* d_cost, float* d_target2, float* d_min, float* d_max, float* h_cost, void* randStates, int dim, int popSize, int maxGenerations, int CR, float F, void* costArgs, float* h_output){  // @arg CR  must be given as value between [0,999]
	int power32 = ceil(popSize / 32.0) * 32;  // printf("\x1b[35m%s\x1b[0m\n",__func__);
	...
	generateRandomVectorAndInit<<<1, power32>>>(d_target, d_min, d_max, d_cost, costArgs, (curandState_t*)randStates, popSize, dim, clock());  gpuErrorCheck(cudaPeekAtLastError());  // cudaMemcpy(d_target2, d_target, sizeof(float) * dim * popSize, cudaMemcpyDeviceToDevice);
	...
	for(int i = 1; i<=maxGenerations; i++){  // std::cout << i << ": generation = \n";  printCudaVector(d_target, popSize * dim);  std::cout << "cost = ";  printCudaVector(d_cost, popSize);  std::cout << std::endl;
		evolutionKernel<<<1, power32>>>(d_target, d_trial, d_cost, d_target2, d_min, d_max, (curandState_t*)randStates, dim, popSize, CR, F, costArgs);  gpuErrorCheck(cudaPeekAtLastError());  // start kernel for this generation

		float* tmp = d_target;  // swap buffers, places newest data into d_target
		d_target   = d_target2;
		d_target2  = tmp;
	}  // end for(generations)

	gpuErrorCheck(cudaDeviceSynchronize());
	gpuErrorCheck(cudaMemcpy(h_cost, d_cost, popSize * sizeof(float), cudaMemcpyDeviceToHost)); // std::cout << "h_cost = {";

	// find min of last evolutions
	int bestIdx = -1;
	float bestCost = FLT_MAX;
	for(int i = 0; i < popSize; i++){
		float curCost = h_cost[i];  // std::cout << curCost << ", ";
		if(curCost <= bestCost){
			bestCost = curCost;
			bestIdx  = i;
		}
	...
	gpuErrorCheck(cudaMemcpy(h_output, d_target+(bestIdx*dim), sizeof(float)*dim, cudaMemcpyDeviceToHost));  // output best minimization.
}

void* createRandNumGen(int size){  // allocate the memory needed for random number generators.
	void* x; gpuErrorCheck(cudaMalloc(&x, sizeof(curandState_t)*size));
	return x;
}
